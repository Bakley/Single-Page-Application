window.addEventListener('load', () => {
  ...
  const router = new Router({
    ...
    page404: (path) => {
      const html = errorTemplate({
        ...
        message: `Sorry pal but the path '/${path}' does not exist on this site`,
      });
      el.html(html);
    },
  });

  // Instantiate api handle
  ...
  const showError = (error) => {
    ...
    const html = errorTemplate({ color: 'blue', title, message });
    el.html(html);
  };

  // Display latest currency rate
  router.add('/', async () => {
